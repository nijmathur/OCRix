...
8. **No singletons**: Services are created fresh by Riverpod providers — do NOT add static singleton factories
...
- **Notifiers**: `Notifier<S>` (sync) or `AsyncNotifier<S>` (async) for state management
...
**Important patterns:**
- All providers are defined in provider files and consumed via `ref.read()` or `ref.watch()` in widgets
- Service providers inject the logger in their factory: `service.setTroubleshootingLogger(ref.read(...))`
- Mutations that modify state must capture `previousState` before the operation and restore on failure
- Background tasks (vectorization, entity extraction) are tracked via `BackgroundTaskNotifier`

**Notifier patterns:**
- Sync state: `class Foo extends Notifier<S> { @override S build() => initial; }`
- Async state: `class Foo extends AsyncNotifier<S> { @override Future<S> build() async { ... } }`
- Family async: `class Foo extends FamilyAsyncNotifier<S, Arg> { @override Future<S> build(Arg arg) async { ... } }`

### Data Models and Serialization

Models in `lib/models/` use **freezed** for immutability and `json_serializable` for serialization:
- Models have `.freezed.dart` and `.g.dart` companion files generated by `build_runner`
- After modifying model classes, run: `dart run build_runner build`
- Pattern: `@freezed abstract class X with _$X { const factory X(...) = _X; }`
- Do NOT add `@JsonSerializable()` to freezed classes — just add `factory X.fromJson(Map<String, dynamic> json) => _$XFromJson(json);`
- Custom methods require `const X._()` private constructor
- All state classes (provider state, models) use freezed — do NOT use Equatable
...
- Image data stored as BLOB in database
- `vendor` field is AES-256 encrypted at rest (v12+); use `DatabaseService.decryptDocumentVendor()` to get plaintext
- Initialization order: DB.initialize() → AuditService.initialize() → DB.setAuditLoggingService(audit)
- AuditDatabaseService must NOT call DatabaseService.initialize() — caller ensures DB is ready first
...
## Architectural Direction (post-review July 2026)

The following decisions were reached through a consensus architecture review. All new code must follow these patterns.

### Non-negotiable rules

1. **No singletons**: Never add `static final _instance = ...; factory Foo() => _instance;`. Riverpod owns the service lifecycle.

2. **No circular init**: `AuditDatabaseService.initialize()` must never call `DatabaseService.initialize()`. Always initialize DB first, then audit, then wire them together.

3. **DI via providers, not constructors**: Services get their dependencies via `setX()` or constructor injection from provider factories — never via `ServiceX()` instantiation inside another service.

4. **Mutations capture previousState**: Every `DocumentNotifier` mutation starts with `final previousState = state;` and restores it in the catch block. No `AsyncValue.error()` on mutation failures.

5. **Background tasks are first-class state**: Use `BackgroundTaskNotifier` (`lib/providers/background_task_provider.dart`) to report task lifecycle. Never fire-and-forget without tracking.

6. **Interfaces for all major services**: Service parameters and fields use interface types (`IDatabaseService`, not `DatabaseService`) except where concrete methods are unavoidable (e.g., `DatabaseExportService` needs `StorageProviderService.getProvider()`).

### Architecture tests

Tests in `test/unit/architecture_regression_test.dart` enforce:
- `DatabaseService` and `AuditDatabaseService` create independent instances (no singleton)
- `BackgroundTaskNotifier.build()` is synchronous
- Provider dependency graph builds without circular errors

These tests must stay green. If an architectural change would break them, reconsider the change first.

### Known limitations / future work

- **`extracted_text` encryption**: FTS5 (`search_index` table) stores plaintext for search. Encrypting `extracted_text` in the main `documents` table requires keeping search_index plain, creating a two-copy model. Not yet implemented.
- **`amount` encryption**: The `amount` column is REAL; encrypting requires adding an `amount_encrypted TEXT` column and migrating. Not yet implemented.
- **Entity fields read path**: `_mapToDocument` now populates entity fields including `vendor` (encrypted). Callers that display vendor must call `DatabaseService.decryptDocumentVendor()` for the decrypted value.
- **Phase 5 modernization** (not done): Records, sealed state hierarchies, extension types, Gradle Kotlin DSL.
