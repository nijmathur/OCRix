...
class GemmaModelService {
  ...
  Future<bool> isModelDownloaded() async {
    try {
      ...
      debugPrint('[GemmaModelService] Error checking model: $e');
      return false;
    }
  }
  ...
  Future<bool> isModelFileAvailable() async {
    try {
      ...
      if (exists) {
        debugPrint('[GemmaModelService] Model file found at: $modelPath');
      }
      return exists;
    } catch (e) {
      debugPrint('[GemmaModelService] Error checking model file: $e');
      return false;
    }
  }
  ...
  Future<String> _copyModelToStorage(String sourceFilePath) async {
    ...
    debugPrint('[GemmaModelService] Model copied successfully to: $destPath');
    return destPath;
  }
  ...
  Future<void> downloadModel(String sourceFilePath) async {
    ...
    try {
      ...
      debugPrint('[GemmaModelService] Model installation failed: $e');
      rethrow;
    } finally {
      _isInstalling = false;
    }
  ...
  Future<void> _installModelFromPath(
    ...
    debugPrint('[GemmaModelService] Model installed successfully');
  }
  ...
  Future<void> initialize() async {
    ...
    try {
      debugPrint('[GemmaModelService] Initializing Gemma model...');

      _model = await FlutterGemma.getActiveModel(maxTokens: maxTokens);

      _isInitialized = true;
      debugPrint('[GemmaModelService] Model initialized successfully');
    } catch (e) {
      debugPrint('[GemmaModelService] Model initialization failed: $e');
      rethrow;
    }
  }
  ...
  Future<String> generateSQL(
    ...
    try {
      ...
      debugPrint('[GemmaModelService] Generated SQL: $sqlQuery');
      return sqlQuery;
    } catch (e) {
      debugPrint('[GemmaModelService] SQL generation failed: $e');
  ...
  Future<void> _reinitializeModel() async {
    try {
      debugPrint('[GemmaModelService] Reinitializing model...');
      await _model?.close();
      _model = null;
      _isInitialized = false;

      await initialize();
      debugPrint('[GemmaModelService] Model reinitialized successfully');
    } catch (e) {
      debugPrint('[GemmaModelService] Model reinitialization failed: $e');
    }
  }
...
Tags: Walmart, groceries, \$45.67, 2024-01-02''';
      ...
      debugPrint('[GemmaModelService] Categorizing document...');
      ...
      debugPrint('[GemmaModelService] Categorized as: $category with tags: $tags');
      ...
      debugPrint('[GemmaModelService] Categorization failed: $e');
  ...
  Map<String, dynamic> _parseCategoryAndTags(String response) {
    ...
    try {
      ...
      debugPrint('[GemmaModelService] Failed to parse structured response: $e');
  ...
  Future<DocumentAnalysis> analyzeDocuments({
    ...
    try {
      ...
      debugPrint('[GemmaModelService] Analysis complete: ${analysis.answer}');
      return analysis;
    } catch (e) {
      debugPrint('[GemmaModelService] Document analysis failed: $e');
  ...
  String _extractSQL(String response) {
    ...
    if (!sql.toUpperCase().contains('FROM')) {
      ...
      if (selectMatch != null) {
        ...
        debugPrint('[GemmaModelService] Auto-completed query: $sql');
      } else {
        throw LLMInferenceException(
