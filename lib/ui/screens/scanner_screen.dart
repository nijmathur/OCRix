    ...
    try {
      ...
      for (int i = 0; i < _capturedPages.length; i++) {
        ...
        try {
          ...
          if (capturedPage.imageBytes == null || capturedPage.imageBytes!.isEmpty) {
            throw Exception('Image bytes are empty for page ${capturedPage.pageNumber}');
          }
          ...
          final ocrResult = await ocrService.extractTextFromImage(capturedPage.imagePath);
          ...
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Text('Warning: Page ${capturedPage.pageNumber} failed OCR: $pageError'),
